            ...
            if facebookError != nil {
                ...
                DataServices.ds.REF_BASE.authWithOAuthProvider("facebook", token: accessToken , withCompletionBlock: {error, authData in
                    ...
                    print ("Error")
                        ...
                        let user = ["provider": authData.provider!, "hjjb":"test"]
                        DataServices.ds.createFireUser(authData.uid, user:user)
                    ...
                    if error.code == STATUS_ACCOUNT_NONEEXIST {
                        ...
                        DataServices.ds.REF_BASE.createUser(email, password: pwd, withValueCompletionBlock: { error, result in
                            if error != nil {
                                ...
                                DataServices.ds.REF_BASE.authUser(email, password: pwd, withCompletionBlock: {err, authData in
                                    let user = ["provider": authData.provider!, "hjjb":"email"]
                                    DataServices.ds.createFireUser(authData.uid, user:user)
                                })
